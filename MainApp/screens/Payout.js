...
export default class SendContacts extends Component {
    ...
    async componentDidMount() {
        ...
        try {
            ...
            const CustomerApiCall = await fetch(Constant.URL+Constant.getTRANSFER,
                {
                    method: "GET",
                    headers,
                });
                ...
                <View style={{ flex: 1 }}>
                    <View style={{ flexDirection: 'row'}}>
                        ...
                        <View style={styles.userdetails}>
                            ...
                            <Text style={{ fontSize: 18, color: '#000',fontFamily: 'Poppins-Light' }}>{rowdata.item.c_ref} </Text>            
                        
                            <Text style={{ fontSize: 10, color: '#000',fontFamily: 'Poppins-Thin' }}>{rowdata.item.a_date} | </Text>
                            <Text style={{ fontSize: 12, color: 'green',fontFamily: 'Poppins-Thin',fontWeight: '800' }}>{rowdata.item.r_amount} | {rowdata.item.means_of_payment} {rowdata.item.beneficiary.ben_bank!=null ? ' ('+rowdata.item.beneficiary.ben_bank+')' : ''}    </Text>
                           
                          <Text  style={{ fontSize: 10,color: 'red'}}> {rowdata.item.cur_from} - {'>'} {rowdata.item.cur_to}</Text>
                        </View>
                    </View>
                </View>
            </TouchableOpacity>
