...
export default class RateRmd extends Component {
    constructor(props) {
        super(props)
        this.state = {
    ...
    async GetRateSet(rate_type) {
        ...
        try {
            ...
            while (i <= (calculated_rate + limit_v)) {
                ...
                i = Constant.naiveRound((i),decima_v_from)
            ...
            //Ascending 
            z = Constant.naiveRound(calculated_rate,decima_v_from)
            while (z <= (calculated_rate + limit_v)) {

                //Set limit for loop
                if (floats.length === limit_v + limit_v) {
                    break;
                }
                z = Constant.naiveRound((z),decima_v_from)
                floats.push(z.toFixed(2));
                z = z + sum_v;
            }
    ...
    async GetRateSetTO(rate_type) {
        ...
        try {
            ...
            if (rate_cal_type_to == 2) {
                ...
                for (var i = 0; i < split.length; i++) {
                    floats2.push(split[i])
                }
                this.setState({div_float: floats2})
            ...
            console.log("from_dollar",to_dollar)
            var i,limit_v = Number(dataSource.data.limit_v_to)
            ...
            var calculated_rate = current_rate
            ...
            while (i <= (calculated_rate + limit_v)) {
                //Set limit for loop
                if (floats2.length === limit_v) {
                    break;
                }

                i = Constant.naiveRound((i),decima_v_to)
                floats2.push(i.toFixed(2));
                i = i - sum_v;
            }
            floats2.shift()

            //Ascending 
            z = Constant.naiveRound(calculated_rate,decima_v_to)
            while (z <= (calculated_rate + limit_v)) {

                //Set limit for loop
                if (floats2.length === limit_v + limit_v) {
                    break;
                }
                z = Constant.naiveRound((z),decima_v_to)
                floats2.push(z.toFixed(2));
                z = z + sum_v;
            }


            floats2.sort(function (a,b) {
                return a - b;
            });

            this.setState({div_float: floats2})

            console.log("floats",floats2)
            //END GET RATE
    ...
    onPressB = () => {
        ...
        //Validate Rate Position
        if (this.state.rate_vPosition != this.state.rate_v2Position) {
            Alert.alert("Wrong receiving rate. Select the same position as sending rate");
            this.setState({rate_v2: 0})
            return false;
        }
        ...
        return (
            <View style={{flex: 1,backgroundColor: Theme.bgcolor}}>
                ...
                <View style={styles.logContainer}>
                    ...
                    <Text style={styles.logintxt}>RMD Rate Calculation  </Text>
                    ...
                    <View style={{flexDirection: 'row',alignItems: 'center',borderWidth: 1,margin: 15,marginTop: 5,paddingHorizontal: 10}}>
                        <Picker style={{flex: 0.9,paddingLeft: 150}}
                            ...
                            onValueChange={(itemValue,itemPosition) => {
                                this.setState({rate_v: itemValue,toIndex: itemPosition,rate_vPosition: itemPosition})
                    ...
                    {/* Rate Division */}
                        ...
                        <View style={{flexDirection: 'row',alignItems: 'center',borderWidth: 1,margin: 15,marginTop: 5,paddingHorizontal: 10}}>
                            ...
                            <Picker style={{flex: 0.9,paddingLeft: 150}}
                                ...
                                onValueChange={(itemValue,itemPosition) => {
                                    this.setState({rate_v2: itemValue,toIndex: itemPosition,rate_v2Position: itemPosition,rate_type: 0})
