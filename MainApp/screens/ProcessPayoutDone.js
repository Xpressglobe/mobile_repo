...
export default class PayoutDone extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            Amessage: "",
            Smessage: "",
            tableHeadRQ: [, 'Agent',  'Customer'],
            tableDataRQ: [
                [props.navigation.getParam('agent_name', '')  , this.props.navigation.getParam('customer_name', '')],
               
            ],
            tableHead: [, 'Transaction ID', 'Date'],
            tableData: [
               [this.props.navigation.getParam('c_ref', '0'), this.props.navigation.getParam('a_date', '0')],
            ],
            RtableHead: ['Beneficiary'],
            RtableData: [
                [this.props.navigation.getParam('beneficiary_ben_bank', '0')],
                [this.props.navigation.getParam('beneficiary_ben_name', '0')],
                [this.props.navigation.getParam('beneficiary_ben_acct', '0')],
    ...
    onPressProcess  = async() => {
        ...
        if (pin.length != 4) {
            ...
            // post method
            fetch(Constant.URL + "payouts/" + this.state.agent_acct_id + "/move-to-processing", {
                method: 'POST',
                headers,
                body: JSON.stringify({
                ...
                .then((result) => {
                    ...
                    if (result.message != "") {
                        ...
                        this.setState({ spinner: false, Sshow: true, Smessage: result.message });

                    } else {

                        this.setState({ spinner: false, show: true, Amessage: "Unexpected Error, Contact Admin!" });
                ...
                <ScrollView>
                    <View style={styles.AmountCon}>
                        ...
                        <View style={styles.rowcenter}>
                            ...
                            <Text style={{ color: "#FFF", fontSize: 30, paddingLeft: 5 }}>{this.state.to} {Constant.numberFormate( bal)}</Text>

                        </View>
                     {/* <Text style={styles.updateSty}>{this.state.agent_name}</Text> */}
                    </View>
                    ...
                    <View style={styles.container}>
                        <Table borderStyle={{ borderWidth: 1 }}>
                            <Row data={this.state.tableHeadRQ} flexArr={[1, 1, 1]} style={styles.head} textStyle={styles.text} />
                            <TableWrapper style={styles.wrapper}>
                                <Rows data={this.state.tableDataRQ} flexArr={[1, 1, 1]} style={styles.row} textStyle={styles.text} />
                            </TableWrapper>
                        </Table>
                    </View>
...
<View style={{flexDirection: 'row',alignItems: 'center',borderWidth: 1,margin: 15,marginTop: 5,paddingHorizontal: 15}}>
                            ...
                            <Picker style={{flex: 0.9,paddingLeft: 150}}
                                ...
                                <Picker.Item label="Select an Action" value="0" />
                                <Picker.Item label="Transfer" value="tansfer" />
                        ...
                        {/* onPress={this.onPressProcess} */}
                            <TouchableOpacity style={{ paddingVertical: 10, backgroundColor: '#020cab', marginTop: 20, borderRadius: 50, marginHorizontal: 20, height: 50 }} >
                                <Text style={{ color: '#FFF', textAlign: 'center', fontSize: 16, fontFamily: 'Poppins-Bold', }}>Process </Text>
                            </TouchableOpacity>
